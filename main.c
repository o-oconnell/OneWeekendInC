...
vec3 at_nopointer_noPointerArgs(ray r, double t) {
    vec3 mulResult = mul_nopointer_noPointerArgs(t, r.direction);

    return plus_nopointer_noPointerArgs(r.origin, mulResult);
}
...
double hit_sphere_alt(vec3 center, double radius, ray r, double ray_tmin, double ray_tmax, hit_record *rec) {
    ...
    rec->t = root;
    vec3 atval = at_nopointer_noPointerArgs(r, rec->t);
...
int main() {
    ...
    //             if (choose_mat < 0.8) {
    //                 vec3 albedo = vec_mul_noPointer(random_vec3(), random_vec3());
    ...
    vec3 u = unit_vector_noPointer(cross(vup, w));
    vec3 v = cross(w, u);
